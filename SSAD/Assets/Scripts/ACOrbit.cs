...
using DifficultyLibBuild;

public class ACOrbit : MonoBehaviour
{
    public DiffClass currentClass;
    private Score scoreNetwork;
        ...
        public int getScoreDB()
	{
        ...
        score = rankResult[0][1].ToInt32();

        return score;
    }

    public DiffClass UpdateDiffML(){
        scoreNetwork = new Score();

        double[] inputArr =  new double[2];
        inputArr[0] = Progress.unlockedPlanet*3;
        inputArr[1] = getScoreDB()/1000;
        
        Debug.Log("-----------------------------");
        Debug.Log(inputArr[0]);
        Debug.Log(inputArr[1]);

        return currentClass = (DiffClass)scoreNetwork.Classify (inputArr);
    }
}
